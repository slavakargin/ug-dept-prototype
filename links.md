# Link registry (dept prototype)

```yaml
- id: UNIV_UG_LANDING
  url: https://slavakargin.github.io/ug-university-prototype/
  scope: ug-internal
  why: University-side prototype landing page for stable UG content.
  owner: Math UG Program
  last_checked: 2025-12-24

- id: BU_ACADEMIC_GUIDE_MATH_BA
  url: https://catalog.binghamton.edu/preview_program.php?catoid=5&poid=1025&returnto=209
  scope: university-external
  why: Official BA requirements (catalog-year specific; update annually).
  owner: Math UG Program
  last_checked: 2025-12-24

- id: BU_ACADEMIC_GUIDE_MATH_BS
  url: https://catalog.binghamton.edu/preview_program.php?catoid=5&poid=1026&returnto=209
  scope: university-external
  why: Official BS requirements (catalog-year specific; update annually).
  owner: Math UG Program
  last_checked: 2025-12-24

- id: BU_ACCELERATED_DEGREE_PROGRAMS
  url: https://www.binghamton.edu/admissions/graduate/apply/accelerated-degree.html
  scope: university-external
  why: Official accelerated/4+1 programs overview.
  owner: Math UG Program
  last_checked: 2025-12-24

- id: BU_HARPUR_MAJOR_MINOR_ADVISING_TEMPLATES
  url: https://www.binghamton.edu/harpur/advising/major-minors/
  scope: university-external
  why: Harpur Advising page hosting 4-year plan templates.
  owner: Math UG Program
  last_checked: 2025-12-24
  ```
